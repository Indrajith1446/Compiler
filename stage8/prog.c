...
struct Classtable * makeclassnode(char *name,char *parentname)
{
    ...
    if(temp->parent!=NULL)
    {
      ...
      parent=temp->parent;
      temp->Memberfield=parent->Memberfield;
      temp->fieldcount=parent->fieldcount;
      temp->methodcount=parent->methodcount;
    }
    return temp;
}
  ...
  switch(*(t->nodetype))
  {
    ...
    case 'I': gn=t->left->Gentry;
              ...
              if(*(t->left->nodetype)=='V')
                printf("\n\nyes var %s %s\n",t->left->varname,t->left->Gentry->name);
              else
              {
                if(*(t->left->nodetype)=='l')
                {
